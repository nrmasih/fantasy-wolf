...
class FrontendController extends Controller {
    ...
    public function register(Request $request) {
        ...
        return view('front.register', compact('faqs'));
    }

    public function resetPassword(Request $request, $email) {
        return view('front.reset-password', compact('email'));
    }
...
// -------------------------------- CONTACT SECTION ----------------------------------------
    ...
    public function aboutus() {
        $b1 = Page::where('slug','about-us')->where('id',1)->first();
        return view('front.about-us', compact('b1'));
    }

    public function offers() {
        $this->code_image();
        $ps = DB::table('pages')
                ->leftJoin('banners', 'pages.slug', '=', 'banners.page')
                ->where('pages.slug', 'offers')
                ->first();
        $b1 = Banner::where('page', 'offers')->where('type', 'B1')->get();
        return view('front.offers', compact('b1', 'ps'));
    }
    ...
    public function policies() {
        ...
        $ps = DB::table('pages')
                //->leftJoin('banners', 'pages.slug', '=', 'banners.page')
                ->where('pages.slug', 'policies')
                ->first();
        $ext = Banner::where('page', 'policies')->where('type', 'Ext')->get();
        return view('front.policies', compact('ext', 'ps'));
    }
    ...
    public function help() {
        ...
        $ps = DB::table('pages')
                //->leftJoin('banners', 'pages.slug', '=', 'banners.page')
                ->where('pages.slug', 'help')
                ->first();
        $ext = Banner::where('page', 'help')->where('type', 'Ext')->get();
        return view('front.policies', compact('ext', 'ps'));
    }
    ...
    public function ourProducts() {
        ...
        $ps = DB::table('pages')
                ->leftJoin('banners', 'pages.slug', '=', 'banners.page')
                ->where('pages.slug', 'our-products')
                ->first();
        $b1 = Banner::where('page', 'our-products')->where('type', 'B1')->get();
        $res = Banner::where('page', 'our-products')->where('type', '!=', 'B1')->where('part', 'P1')->get();
        return view('front.our-products', compact('res', 'b1', 'ps'));
    }

    public function shippingReturns() {
        $this->code_image();
        $ps = DB::table('pages')
                ->where('pages.slug', 'shipping-returns')
                ->first();
        $ext = Banner::where('page', 'shipping-returns')->where('type', 'Ext')->get();
        return view('front.policies', compact('ext', 'ps'));
    }
    ...
    public function corporate() {
        $this->code_image();
        $ps = DB::table('pages')
                ->where('pages.slug', 'corporate-gifts')
                ->first();
        $b1 = Banner::where('page', 'corporate-gifts')->where('type', 'B1')->first();
        $b2 = Banner::where('page', 'corporate-gifts')->where('type', 'B2')->first();
        $b3 = Banner::where('page', 'corporate-gifts')->where('type', 'B3')->first();
        return view('front.corporate-gifts', compact('b1', 'b2', 'b3', 'ps'));
    }
    ...
    public function wholesale() {
        ...
        $ps = DB::table('pages')
                ->where('pages.slug', 'corporate-gifts')
                ->first();
        //$ext =  Banner::where('page','wholesale')->where('type','Ext')->get();    
        return view('front.wholesale', compact('ext', 'ps'));
    }
...
// -------------------------------- PRINT SECTION ENDS ----------------------------------------
    ...
    public function formRetailer(Request $request) {
        ...
        $data = new Wholesale();
        $input = $request->all();
        if ($request->ftype == 'retailer') {
            if (!empty($request->organization_type)) {
                $input['organization_type'] = implode(",", $request->organization_type);
            }
            if ($file = $request->file('storeImage1')) {
                $name = time() . $file->getClientOriginalName();
                $file->move('assets/images/wholesale', $name);
                $input['storeImage1'] = $name;
            }
            if ($file = $request->file('storeImage2')) {
                $name = time() . $file->getClientOriginalName();
                $file->move('assets/images/wholesale', $name);
                $input['storeImage2'] = $name;
            }
            if ($file = $request->file('storeImage3')) {
                $name = time() . $file->getClientOriginalName();
                $file->move('assets/images/wholesale', $name);
                $input['storeImage3'] = $name;
            }
            if ($file = $request->file('storeImage4')) {
                $name = time() . $file->getClientOriginalName();
                $file->move('assets/images/wholesale', $name);
                $input['storeImage4'] = $name;
            }
        }
        $res = $data->fill($input)->save();
        if (!empty($res)) {
            return response()->json(['status' => 1, 'message' => 'Form submitted successfully !', 'response' => $input], 200);
        } else {
            return response()->json(['status' => 0, 'message' => 'Something problem  !', 'response' => ''], 200);
        }
    }
